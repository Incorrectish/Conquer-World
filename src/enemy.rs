...
impl Enemy {
    ...
    pub fn kill(world: &mut World, index: usize) {
        ...
        for tile in &world.enemies[index].pos {
    ...
    pub fn move_enemy_with_deltas(index: usize, world: &mut World) {
        let (delta_x, delta_y) = (
            world.enemies[index].pos[0].x as i32 - world.player.pos.x as i32,
            world.enemies[index].pos[0].y as i32 - world.player.pos.y as i32,
        );
        ...
        for i in 0..world.enemies[index].pos.len() {
            let (a_new_pos, _) = World::new_position(
                world.enemies[index].pos[i],
                direction,
                world,
        ...
        if new_pos.contains(&world.player.pos) {
            ...
            for i in 0..new_pos.len() {
                World::update_position(
                    world,
                    world.enemies[index].pos[i],
                    (new_pos[i], world.world_position),
                );
            }
    ...
    pub fn move_enemy(index: usize, world: &mut World) {
        ...
        for _ in 0..world.enemies[index].speed {
            if let Some(new_pos) = travel_path.pop_front() {
                if Self::match_color(&world.enemies[index].color, &tile::CHASING_ENEMY) {
                    ...
                    if new_pos == world.player.pos {
                        ...
                        // simply updates the render queue
                        World::update_position(world, world.enemies[index].pos[0], (new_pos, world.world_position));
                        world.enemies[index].pos[0] = new_pos;
                    }
                } else if Self::match_color(&world.enemies[index].color, &tile::BOMBER_ENEMY) {
                    // activate bomber if within range (no movement)
                    if Self::player_within_spaces(&world.enemies[index].pos[0], &world, 2) {
                        world.enemies[index].color = tile::BOMBER_ENEMY_ACTIVATED;
                        let curr_world =
                            &mut world.entity_map[world.world_position.y][world.world_position.x];
                        curr_world.insert(world.enemies[index].pos[0], (world.enemies[index].color, Entity::Enemy));
                        return;
                    }
                    ...
                    let del_x = world.enemies[index].pos[4].x as i16 - world.player.pos.x as i16;
                    let del_y = world.enemies[index].pos[4].y as i16 - world.player.pos.y as i16;
                    ...
                    let positions = world.enemies[index].pos.clone();
                    for tile in &positions {
                        ...
                        for i in 0..world.enemies.len() {
                            if i != index && world.enemies[i].pos.contains(&new_pos) {
                                return;
                            }
                        }
                    }
                    ...
                    if move_x == -1 || move_y == -1 {
                        for i in 0..positions.len() {
                            let new_x = world.enemies[index].pos[i].x as i16 + move_x;
                            let new_y = world.enemies[index].pos[i].y as i16 + move_y;
                            let new_pos = Position::new(new_x as usize, new_y as usize);
                            World::update_position(world, world.enemies[index].pos[i], (new_pos, world.world_position));
                            world.enemies[index].pos[i] = new_pos;
                        }
                    } else {
                        for i in 0..positions.len() {
                            let new_x = world.enemies[index].pos[positions.len()-i-1].x as i16 + move_x;
                            let new_y = world.enemies[index].pos[positions.len()-i-1].y as i16 + move_y;
                            let new_pos = Position::new(new_x as usize, new_y as usize);
                            World::update_position(world, world.enemies[index].pos[positions.len()-i-1], (new_pos, world.world_position));
                            world.enemies[index].pos[positions.len()-i-1] = new_pos;
                        }
                    }
                } else {
                    break;
    ...
    pub fn get_best_path(
        ...
        let enemy_pos = world.enemies[index].pos[0];
    ...
    pub fn get_neighbors(
        ...
        for direction in directions {
            ...
            // neighbors
            if new_pos != world.enemies[index].pos[0]
                && Self::can_travel_to(
    ...
    pub fn create_bomber_explosion(index: usize, world: &mut World) {
        let pos = world.enemies[index].pos[0];
        for i in -2..=2_i16 {
