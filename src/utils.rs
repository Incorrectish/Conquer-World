...
impl Boss {
    ...
    pub fn attack(world: &mut World, index: usize) {
        if world.world_position == BOSS_ROOMS[0] {
            Self::generate_lasers(world, world.bosses[index].laser_amount, index);
    ...
    pub fn generate_lasers(world: &mut World, num_lasers: usize, index: usize) {
    ...
    pub fn draw_lasers(world: &mut World, canvas: &mut Canvas) {
    ...
    pub fn generate_safe_spot(world: &mut World, index: usize) {
        if let Some(spot) = &mut world.boss_safe_spot {
            ...
            while Self::pos_inside_boss(world, Position::new(pos.x + size, pos.y), world.world_position) ||
                ...
                Self::pos_inside_boss(world, Position::new(pos.x, pos.y - size), world.world_position){
                let x = random::rand_range(&mut world.rng, max(0, player_x - 5), player_x + 5) as usize;
                let y = random::rand_range(&mut world.rng, max(0, player_y - 5), player_y + 5) as usize;
                pos = Position::new(x,y);
            }
    ...
    pub fn generate_vulnerable_spot(world: &mut World, index: usize) {
        if let Some(spot) = world.boss_vulnerable_spot {
            return;
        } else {
            if world.bosses[index].vulnerable_time == 0 {
                let offset = world.bosses[index].offset - 1;
                let on_vert = Self::coin_flip(&mut world.rng); // On horizontal?
                let pos_side = Self::coin_flip(&mut world.rng); //On top or left?
                let spot = random::rand_range(&mut world.rng, 0, offset as i16 * 2 - 1) as usize;
